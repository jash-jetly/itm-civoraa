// BIP39 word list (simplified version for demo - in production use the full 2048 word list)
const WORD_LIST = [
  ...
  'isolate', 'news', 'item', 'ivory', 'jacket', 'jaguar', 'jar', 'jazz',
  ...
  'tilt', 'timber', 'time', 'tiny', 'tip', 'tired', 'tnews', 'title',
...
export const getRandomWordPositions = (count: number = 4): number[] => {
  ...
  while (positions.length < count) {
    const randomPosition = Math.floor(Math.random() * 12) + 1; // 1-12
    if (!usedPositions.has(randomPosition)) {
...
// Verify seed phrase words at specific positions
export const verifySeedPhrase = (
  originalPhrase: string[], 
  userInputs: { position: number; word: string }[]
): boolean => {
  return userInputs.every(input => {
    const expectedWord = originalPhrase[input.position - 1]; // Convert to 0-based index
    return expectedWord.toLowerCase() === input.word.toLowerCase().trim();
  });
};